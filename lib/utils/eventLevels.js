...
function sortEvents(evtA, evtB, _ref3) {
  ...
  //allDayAccessor는 종일인지 비종일인지 true, false값을 리턴하는 함수. 리액트빅캘린더 컴포넌트 도입할때 같이 넘겨준다.
  ...
  var results = durA > 1 && durB > 1 ? Math.max(durA, 1) - Math.max(durB, 1) : Math.max(durB, 1) - Math.max(durA, 1);
  var results2 = 0;
  if (evtA.schedule_option === 'all_day' && evtB.schedule_option === 'all_day') {
    results2 = evtA.period_type_num - evtB.period_type_num;
  }

  var results3 = evtA.schedule_idx - evtB.schedule_idx;

  return startSort // sort by start Day first
  // || Math.max(durA, 1) - Math.max(durB, 1)   // events spanning multiple days go first
  || results || results2 || !!(0, _accessors.accessor)(evtB, allDayAccessor) - !!(0, _accessors.accessor)(evtA, allDayAccessor // then allDay single day events
